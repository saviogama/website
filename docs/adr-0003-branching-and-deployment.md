# ADR 0003: Branching and Deployment Workflow
...
The MVP is ready to be organized into commits and branches before publication.
...
The Vercel project should use `main` as the Production Branch. Pull requests and non-production branches may still produce preview deployments, but they should not be considered production releases.
...
The production-only deploy rule cannot be fully enforced by repository files alone when Vercel Git integration is used. The Vercel dashboard must also be configured with `main` as the Production Branch.
